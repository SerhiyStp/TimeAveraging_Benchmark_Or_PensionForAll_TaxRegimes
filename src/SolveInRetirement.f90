module SolveRetirement

    implicit none

contains

    subroutine SolveInRetirement(counter, p4a)
    ...
    subroutine SolveInRetirement2(counter, p4a)
        ...
        do ifc=1,nfc
            ...
            if(it==1) then
                ...
                !Finding optimal capital by golden search
                P1=0.001d0
                nonlab_inc = (k_grid(ik)+Gamma_redistr*0.5d0)*(1d0+r_ret*(1d0-tk))+lumpsum*0.5d0 + Psi_pensionf*p4a
                ...
                !Working 0 hours but not retiring
                P1=0.001d0
                P4=min((k_grid(nk)-0.001d0)/(1d0+tc),((k_grid(ik)+Gamma_redistr*0.5d0)*(1d0+r_ret*(1d0-tk))+lumpsum*0.5d0+Psi_pensionf*p4a+Unemp_benefit)/(1d0+tc))
                do
                    P2 = P1 + ((3.0-sqrt(5.0))/2.0)*(P4-P1)
                    P3 = P1 + ((sqrt(5.0)-1.0)/2.0)*(P4-P1)

                    dum2=((k_grid(ik)+Gamma_redistr*0.5d0)*(1d0+r_ret*(1d0-tk))+lumpsum*0.5d0+Psi_pensionf*p4a+Unemp_benefit-P2*(1d0+tc))/(1d0+mu)
                    if(dum2<0.0001d0) then
                        V2=-999999999d0
                    elseif(dum2>k_grid(nk)-0.001d0) then
                        V2=Uc(P2)
                    ...
                    dum2=((k_grid(ik)+Gamma_redistr*0.5d0)*(1d0+r_ret*(1d0-tk))+lumpsum*0.5d0+Psi_pensionf*p4a+Unemp_benefit-P3*(1d0+tc))/(1d0+mu)
                    if(dum2<0.0001d0) then
                        V3=-999999999d0
                    elseif(dum2>k_grid(nk)-0.001d0) then
                        V3=Uc(P3)
        ...
        do ifc=1,nfcm
            ...
            if(it==1) then
                ...
                !Finding optimal capital by golden search
                P1=0.001d0
                nonlab_inc = (k_grid(ik)+Gamma_redistr*0.5d0)*(1d0+r_ret*(1d0-tk))+lumpsum*0.5d0+Psi_pension*p4a
                ...
                !Working 0 hours but not retiring
                P1=0.001d0
                P4=min((k_grid(nk)-0.001d0)/(1d0+tc),((k_grid(ik)+Gamma_redistr*0.5d0)*(1d0+r_ret*(1d0-tk))+lumpsum*0.5d0+Psi_pension*p4a+Unemp_benefit)/(1d0+tc))
                do
                    P2 = P1 + ((3.0-sqrt(5.0))/2.0)*(P4-P1)
                    P3 = P1 + ((sqrt(5.0)-1.0)/2.0)*(P4-P1)

                    dum2=((k_grid(ik)+Gamma_redistr*0.5d0)*(1d0+r_ret*(1d0-tk))+lumpsum*0.5d0+Psi_pension*p4a+Unemp_benefit-P2*(1d0+tc))/(1d0+mu)
                    if(dum2<0.0001d0) then
                        V2=-999999999d0
                    elseif(dum2>k_grid(nk)-0.001d0) then
                        V2=Uc(P2)
                    ...
                    dum2=((k_grid(ik)+Gamma_redistr*0.5d0)*(1d0+r_ret*(1d0-tk))+lumpsum*0.5d0+Psi_pension*p4a+Unemp_benefit-P3*(1d0+tc))/(1d0+mu)
                    if(dum2<0.0001d0) then
                        V3=-999999999d0
                    elseif(dum2>k_grid(nk)-0.001d0) then
                        V3=Uc(P3)
    ...
    subroutine SolveInRetirement3(counter, p4a)
                    ...
                    do iuf = 1, nu
                        do ixm = 1, nexp
                            ...
                            if(it==1) then
                                ...
                                !Finding optimal capital by golden search
                                P1=0.001d0
                                nonlab_inc = (k_grid(ik)+Gamma_redistr)*(1d0+r_ret*(1d0-tk))+lumpsum+(Psi_pension+Psi_pensionf)*p4a
                                ...
                                P1=0.001d0
                                P4=min((k_grid(nk)-0.001d0)/(1d0+tc),((k_grid(ik)+Gamma_redistr)*(1d0+r_ret*(1d0-tk))+lumpsum+(Psi_pension+Psi_pensionf)*p4a+2d0*Unemp_benefit)/(1d0+tc))
                                do
                                    P2 = P1 + ((3.0-sqrt(5.0))/2.0)*(P4-P1)
                                    P3 = P1 + ((sqrt(5.0)-1.0)/2.0)*(P4-P1)

                                    dum2=((k_grid(ik)+Gamma_redistr)*(1d0+r_ret*(1d0-tk))+lumpsum+(Psi_pension+Psi_pensionf)*p4a+2d0*Unemp_benefit-P2*(1d0+tc))/(1d0+mu)
                                    if(dum2<0.0001d0) then
                                        V2=-999999999d0
                                    elseif(dum2>k_grid(nk)-0.001d0) then
                                        V2=Uc(P2)
                                    ...
                                    dum2=((k_grid(ik)+Gamma_redistr)*(1d0+r_ret*(1d0-tk))+lumpsum+(Psi_pension+Psi_pensionf)*p4a+2d0*Unemp_benefit-P3*(1d0+tc))/(1d0+mu)
                                    if(dum2<0.0001d0) then
                                        V3=-999999999d0
                                    elseif(dum2>k_grid(nk)-0.001d0) then
                                        V3=Uc(P3)
